...
struct IdentityPresentationContractTests {
    ...
    @Test func aHexKeyPublishedAsADisplayNameIsNotRenderedAsAName() {
        let resolved = IdentityPresentation.resolve(
            accountIdHex: Fixture.hex,
            knownName: Fixture.otherHex
        )

        #expect(resolved.source == .npub)
        #expect(!containsHex(resolved.text, Fixture.otherHex))
        #expect(resolved.text == IdentityFormatter.short(Fixture.npub))
    }

    @Test func aHexKeyDisplayNameFallsBackToGenericCopyWhenTheAccountIdIsUnusable() {
        let resolved = IdentityPresentation.resolve(
            accountIdHex: nil,
            knownName: Fixture.hex
        )

        #expect(resolved.source == .unknown)
        #expect(!containsHex(resolved.text, Fixture.hex))
    }

    @Test func onlyExactlyThirtyTwoByteAsciiHexNamesAreRejected() {
        let allowed = [
            "Deadbeef",
            String(repeating: "ab", count: 31),
            String(repeating: "ab", count: 33),
            String(repeating: "\u{FF10}", count: 64),
        ]

        for name in allowed {
            let resolved = IdentityPresentation.resolve(accountIdHex: Fixture.hex, knownName: name)
            #expect(resolved.source == .name, "\(name.prefix(12))")
            #expect(resolved.text == name, "\(name.prefix(12))")
        }

        let uppercased = IdentityPresentation.resolve(
            accountIdHex: Fixture.hex,
            knownName: Fixture.otherHex.uppercased()
        )
        #expect(uppercased.source == .npub)
    }
