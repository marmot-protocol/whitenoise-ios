...
struct MarmotHandleLockdownTests {
    ...
    private static let seamFiles: Set<String> = [
        ...
        "whitenoise-ios/Core/RuntimeLifecycle.swift",
        "whitenoise-ios/Core/NotificationCoordinator.swift",
        "whitenoise-ios/Core/MarmotClient.swift",
    ]
    ...
    @Test func scannerSeesRawHandleInTheSeamFiles() throws {
        for relative in Self.seamFiles {
            let url = Self.appTargetRoot.appendingPathComponent(
                String(relative.dropFirst("whitenoise-ios/".count))
            )
    ...
    /// Absolute path of the `whitenoise-ios/` app source root, derived from this
    /// test file's location (`whitenoise-iosTests/MarmotHandleLockdownTests.swift`
    /// → repo root → `whitenoise-ios`).
    private static var appTargetRoot: URL {
        URL(filePath: #filePath)
            .deletingLastPathComponent()   // whitenoise-iosTests/
            .deletingLastPathComponent()   // repo root
            .appendingPathComponent("whitenoise-ios")
    }

    private struct SourceFile {
        let url: URL
        /// Repo-relative path, e.g. `whitenoise-ios/Chats/ChatsListView.swift`.
