...
nonisolated enum IdentityPresentation {
    ...
    static func resolve(
        ...
        // suppress the npub. A name that is itself a 32-byte key is rejected so
        // a peer cannot publish raw hex as their kind:0 name and have it render
        // as one.
        if let name = ContentSanitizer.displayName(knownName), !Hex.is32Bytes(name) {
